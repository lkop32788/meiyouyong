...
export interface AskUserParams {
  ...
  questions: {
    ...
    question: string
    /** Short label (max 18 chars) displayed as a chip/tag */
    header?: string
...
/**
 * A single entry in the read_files `paths` list: either a plain file path, or a
 * windowed read of a specific line range (used with the agent definition's
 * `windowedFileReads: true` opt-in).
 */
export type FilePathEntry =
  ...
  | {
      ...
      offset?: number
      /** Maximum number of lines to read (defaults to 2,000). */
      limit?: number
    }

/**
 * Read the multiple files from disk and return their contents. List several paths to read several files in one call. Large files are cut off after 2,000 lines. Don't read a large file whole: use code_search first and then read a window around the match with { path, offset, limit } entries (requires the `windowedFileReads` agent-definition opt-in).
 */
...
/**
 * A reference to a tracked click URL from a prior gravity_index search result.
 * The runtime verifies the selection and substitutes the exact stored URL.
 */
export type GravityIndexLinkRef = {
  source: 'gravity_index'
  /** The exact search_id returned by gravity_index. */
  search_id: string
  /** The exact slug of the service selected from that search. */
  service_slug: string
}
...
export interface RenderUiParams {
  ...
  widget: {
    ...
    text: string
    /** The http(s) URL to open, or a gravity_index reference (with the exact search_id and service_slug) that the runtime resolves to the exact tracked click URL. */
    link: string | GravityIndexLinkRef
...
export interface SetOutputParams {
  /** The output value to set. A plain object, passed back to the parent agent when spawned. */
